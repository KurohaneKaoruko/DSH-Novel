...
【去 AI 味 · 落笔即防】（创作文本从第一句起就按此执行：写干净再交稿，不做「先带 AI 味、事后去味」的二道工序）：

写法（动笔的顺序）：
1. 先想画面再动笔：谁、在哪、做什么、心里是什么情绪——然后只写看得见听得着的部分：动作、对话、环境、五感细节；情绪藏在画面里，让读者自己读出来。用名词和动词承载，形容词副词能省则省。
2. 写完一段扫一段：本段一旦出现套话、情绪标签、词藻堆砌，当场重写这一段，再写下一段。
3. 交稿前整篇快扫：从头把漏网的 AI 腔句改掉，只输出改定稿；宁可多花一次自检，不带病交稿。

红线（逐条禁，撞上任何一条就重写该句）：
1. 禁用万能套话：如「在这个……的世界里」「他不知道的是」「时间仿佛凝固」「空气瞬间安静」「眼底闪过一丝」「嘴角勾起一抹」「这一刻，世界都安静了」「然而事情远没有这么简单」「不知过了多久」「一室寂静」。
2. 禁情绪标签与模板化生理反应：不写「他很生气/悲伤/紧张」，也不写「心中一沉」「瞳孔骤缩」「倒吸一口凉气」「后背发凉」「心脏漏跳一拍」「指尖微颤」这类套路化反应——改写成该人物独有的动作、眼神、语气与小动作。
3. 禁堆砌：不连续堆四字词，不用排比撑场面，不滥用「缓缓」「轻轻」「微微」；「一丝」「一抹」「几分」等万能量词一律砍掉。
4. 比喻要节制且换着写：一段最多一个比喻、只留最准的；引导词必须轮换（像/仿佛/如同/似/宛如/恰似……轮着用），严禁整篇反复出现「……，像……」同一句式——同一章里同一引导词的比喻不超过 3 次；能直写就不用比喻，删掉比喻句意不损失的，删。
5. 少解释多留白：别在每个动作后补心理分析，内心戏只在关键处给一两句；留白让读者自己脑补。
6. 对话像人话：口语化、有口癖、有停顿、有答非所问，别让每个人物都像念书面报告；不同人物说话必须有区分度（用词、句长、口癖各不相同，遮住名字也能认出是谁在说）。
7. 结构长短错落：长短句、长短段交替；允许平淡过渡，别陷入「动作—心理—结论」的循环；结尾不强行升华总结，钩子靠剧情不靠抒情。
8. 视角信息边界（防上帝视角）：角色只能知道自己经历过、被告知过、或能合理推断的信息。设定、事件、他人动机若角色不可能知道：要么不写、要么由叙述旁白向读者交代（角色仍不知情）、要么留到后文自然揭晓——绝不允许角色说出无来源的信息，或对超出当前剧情的事直接做出反应。
9. 正文禁止 markdown 符号、emoji、成串破折号与解释性插入语。

对照示例（✗ 为 AI 腔，✓ 为参考写法）：
- ✗「他眼底闪过一丝不悦，嘴角勾起一抹危险的弧度。」✓「他把筷子往桌上一搁，没再看她。」
- ✗「空气瞬间安静下来，时间仿佛在这一刻凝固。」✓「没人接话。厨房里只有抽油烟机还在响。」
- ✗「她感到一阵难以言喻的悲伤涌上心头。」✓「她把手机扣在桌上，去阳台站了很久。」
- ✗「他的声音冷得像冰，眼神锐利得像刀。」（同一句式连用）✓ 直写：「他没应声，把判决书推了回来。」
- ✗「林远一眼就认出，这是三百年前灭门的沈家信物。」（他无从得知）✓ 只写他把玉佩收进怀里，来历不提；或用旁白：「玉佩内侧刻着一个早已失传的姓氏。」角色本人并不知道这意味着什么。`;
...
5. 润色产出的文本按【去 AI 味 · 落笔即防】清理一遍：套话、情绪标签、堆砌在润色中一并去掉，交出的就是干净稿。
...
（逐条列出主要修改及原因，每条一行，简明）`,
    ...
    tool({
      ...
      parameters: {
        ...
        style_ref: { type: 'string', description: '文风参考（可选）：文风画像或风格指南（novel_style_guide 的输出），成稿语言风格向其对齐，不模仿其内容' },
      },
      system: (args) => `${commonSystem}
...
7. 按【去 AI 味 · 落笔即防】执行：先想画面再落笔，写完一段扫一段，输出前整篇快扫一遍——交出的就应该是干净稿。`,
      user: (args) => compose([
        ...
        field('文风参考（对齐语言风格，不模仿内容）', args.style_ref),
      ]),
      opts: { maxTokens: 8000 },
    });
...
4. 遵守大纲与设定，保持叙事视角一致。
5. 若提供「文风参考」：句式节奏、用词雅俗、描写密度与对话风格向其对齐，但不模仿其具体情节与内容。
6. 场景结尾留钩子或情绪余韵。直接输出正文，不输出说明。
7. 按【去 AI 味 · 落笔即防】执行：先想画面再落笔，写完一段扫一段，输出前整篇快扫一遍——交出的就应该是干净稿。`,
      user: (args) => compose([
...
6. 正文部分按【去 AI 味 · 落笔即防】执行：先想画面再落笔，写完一段扫一段，交出的就应该是干净稿。`,
      user: (args) => compose([
...
4. 改写后的文本按【去 AI 味 · 落笔即防】快扫一遍再输出，不把旧稿的 AI 腔带进新稿。`,
      user: (args) => compose([
        field('原文', args.text),
...
6. 优化后的片段按【去 AI 味 · 落笔即防】扫一遍再输出，尤其防止把口语又改成书面腔。`,
      user: (args) => compose([
...
2. 输出「文风画像」：一句话概括 + 分项特征 + 可模仿要点。画像保持 200 字以内的紧凑形式，可直接粘贴给 novel_write_chapter / novel_continue / novel_scene_write 的「文风参考」参数，成稿会自动向该风格对齐。

创建模式：
3. 产出风格指南：目标读者、总体语调、词汇表（偏好词/禁用词）、句式建议、描写原则、对话风格、情绪节奏、禁忌清单、风格参考例句（原创）。指南同样可回流给成稿工具作「文风参考」。`,
      user: (args) => compose([
...
4. 方案之间差异化明显，避免同质；遵守「约束」。`,
    ...
    tool({
      name: 'novel_deai',
      description: '去 AI 味：给外来文本或旧稿做「AI 腔」体检与重写——逐句找出套话、模板句、过度心理分析、形容词堆砌、书面腔对话与均匀结构并改写，保留情节与人设。正式创作请直接用写作工具（成稿时已落笔即防），本工具用于处理粘贴的外部文本、AI 旧稿或最终兜底。',
      timeoutMs: 180000,
      parameters: {
        ...routeParams,
...
你现在担任网文「去 AI 味」专项编辑，任务只有一个：把 AI 腔文字改成自然、有烟火气的网文。（正常创作流程在落笔时就按红线防 AI 味；你处理的是存量文本与外部稿件。）

操作步骤：
1. 通读全文，逐句标记 AI 腔特征：万能套话、模板句、模板化生理反应（心中一沉/瞳孔骤缩/倒吸凉气）、过度心理分析、形容词/副词堆砌、万能量词（一丝/一抹/几分）、连用比喻与同一引导词高频重复（整篇反复「……，像……」）、上帝视角越界（角色说出了不可能知道的信息）、书面腔对话与人物语气无区分、均匀无变化的段落结构与节奏、结尾强行升华。
2. 按「mode」执行：
...
时间用故事内纪年/相对时间（如「开篇当日」「三天后」），信息不明处标【待定】。按故事内时间先后排序。只输出表格，不要其他说明。`;
    ...
    tool({
      ...
      description: '单章成稿流水线：按本章细纲+写前简报一次性产出整章定稿——写作时即按「落笔即防」规范边写边查（草稿→（自查）→修订只做兜底），可选直接保存进作品工程。写正文章节的首选工具，长篇推进时配合 novel_briefing 与 novel_archive 使用。',
      timeoutMs: 600000,
      parameters: {
        ...routeParams,
...
7. 全程执行【去 AI 味 · 落笔即防】：先想画面再落笔，写完一段扫一段；全文写完后再整篇快扫一遍才输出——初稿就必须是去净 AI 腔的干净稿，后续修订只做兜底，不是擦洗工序。`;
        const finisherSystem = `${commonSystem}

你现在担任网文终稿写手。初稿已按红线边写边防，你的任务是兜底终检：只改必须改的，不要推倒重写。
...
八、去 AI 味（落笔时避免，不是写完再补）
- 动笔前先想画面：只写看得见听得着的动作、对话与环境细节，情绪藏在画面里让读者自己读出来；
- 写完一段扫一段：出现套话、情绪标签、堆砌，当场重写该段再往下写；
- 交稿前整篇快扫一遍兜底：所有成稿工具（单章成稿/续写/场景写作/润色/改写）的输出都应该是干净稿，不做「先带 AI 味、事后去味」的二道工序；
- 比喻快扫：检查「像/仿佛/如同」是否同一句式高频重复——同一段最多一个比喻、引导词要轮换，删掉不影响句意的比喻；
- 视角快扫：检查角色是否说出了不可能知道的信息（上帝视角越界）——越界信息改成旁白向读者交代，或删掉留到后文；
- 对话快扫：遮住名字读对话，认不出是谁在说就重写；
