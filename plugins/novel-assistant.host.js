...
return {
  ...
  apply(ctx) {
    ...
    async function resolveRoute(args, exec) {
    ...
    async function generate(system, user, args, exec, opts) {
      const route = await resolveRoute(args, exec);
      const messages = [{
        id: `novel-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`,
        role: 'user',
        content: [{ type: 'text', text: user }],
        source: { kind: 'plugin', plugin: 'novel-assistant' },
      }];
      const options = {
...
4. 方案之间差异化明显，避免同质；遵守「约束」。`,
    ...
    const TEXT_EXT = new Set(['txt', 'md', 'markdown', 'json', 'jsonl', 'yaml', 'yml', 'js', 'mjs', 'cjs', 'ts', 'tsx', 'jsx', 'py', 'html', 'htm', 'css', 'scss', 'less', 'csv', 'tsv', 'log', 'xml', 'toml', 'ini', 'conf', 'cfg', 'sh', 'ps1', 'bat', 'cmd', 'tex', 'rst', 'adoc', 'org', 'vue', 'sql', 'env', 'properties', 'lock']);
    const MAX_BYTES = 5 * 1024 * 1024;

    // 解析根目录：客户端传入的 root > 会话 header.cwd（工作区）> 部署兜底根
    async function resolveBase(args) {
      const sid = args && typeof args.sessionId === 'string' ? args.sessionId : '';
      let session;
      if (sid && sessions !== undefined) {
        try { session = sessions.get(sid); } catch (e) { session = undefined; }
      }
      let sessionCwd;
      if (session && session.header && typeof session.header.cwd === 'string' && session.header.cwd) {
        sessionCwd = session.header.cwd;
      }
      const given = args && typeof args.root === 'string' && args.root ? args.root : '';
      const root = given || sessionCwd || (sp.workspaceRoot && sp.workspaceRoot.length ? sp.workspaceRoot : '.');
      const baseTarget = await fs.resolve(root);
      return { root, baseTarget, session };
    }

    // 列出工作区（或子目录）下的目录与文本文件
    harness.handle('nreader:list', async (args) => {
      const { root, baseTarget } = await resolveBase(args);
      const dir = args && typeof args.dir === 'string' ? args.dir.replace(/^\/+|\/+$/g, '') : '';
      const target = dir ? await fs.resolve(dir, { cwd: root }) : baseTarget;
      if (!fs.contains(baseTarget, target)) throw new Error('路径超出工作区');
      ...
      for (const e of entries) {
        ...
        if (e.type === 'directory') {
          rows.push({ name: e.name, type: 'directory', size: 0, path: rel });
        } else {
          const dot = e.name.lastIndexOf('.');
          const ext = dot >= 0 ? e.name.slice(dot + 1).toLowerCase() : '';
          if (ext && !TEXT_EXT.has(ext)) continue;
          rows.push({ name: e.name, type: 'file', size: e.size || 0, path: rel });
        }
      }
      rows.sort((a, b) => (a.type === b.type ? (a.name < b.name ? -1 : a.name > b.name ? 1 : 0) : a.type === 'directory' ? -1 : 1));
      return { root, dir, rows };
    });

    // 读取一个文本文件
    harness.handle('nreader:read', async (args) => {
      ...
      if (info === undefined) throw new Error(`文件不存在: ${path}`);
      if (info.type !== 'file') throw new Error(`不是普通文件: ${path}`);
      if (info.size !== undefined && info.size > MAX_BYTES) throw new Error(`文件超过 5MB，窗口内不支持打开: ${path}`);
      const content = await fs.readText(target);
      return { path, name: path.split('/').pop() || path, content, size: info.size || content.length };
    });

    // 写回文本文件（沙箱策略以会话 cwd 为工作区边界）
    harness.handle('nreader:write', async (args) => {
      const path = args && typeof args.path === 'string' ? args.path : '';
      const content = args && typeof args.content === 'string' ? args.content : '';
      if (!path) throw new Error('缺少文件路径');
      const { root, baseTarget, session } = await resolveBase(args);
      const target = await fs.resolve(path, { cwd: root });
      if (!fs.contains(baseTarget, target)) throw new Error('路径超出工作区');
