...
1. 把仓库中的整个 `novelist/` 目录复制到 `$HOME/.dsh/.agent-presets/novelist/`，包含：
   ...
   - `plugins/novel-tools.mjs`（6 个零模型调用工具）
   - `skills/`（7 个方法论 skill 目录：novel-prose-standards / novel-ai-lexicon / novel-continuity / novel-plotting / novel-craft / novel-analysis / novel-project）
...
3. 新建会话，在预设选择器中选择「**小说助手**」，确认：
   - 工具列表包含 6 个 `novel_*` 工具（novel_lint / novel_briefing / novel_archive / novel_project / novel_import / novel_scan_book）；
   - 技能列表包含 7 个 `novel-*` skill（含 novel-ai-lexicon AI 高频词分级词库）；
   - 系统提示包含「小说助手工作法」路由段（分工原则 + 每章闭环 + skill/工具路由 + 硬纪律）。

## 2. 收尾与使用

- 完成标志：新会话选择「小说助手」后，能加载 novel-* skill、能调用 6 个 novel_* 工具、能直接开始写小说。
...
- **Q：工具调用报错？** 6 个工具均为零模型调用的纯代码工具——报错通常是文件系统/工程文件问题（如未初始化工程），按报错信息处理即可；正文生成由 Agent 本体完成，不经过工具。
- **Q：预设选择器里没有「小说助手」？** 确认 `novelist/` 已完整复制到用户预设根目录（`skills/` 子目录必须随行——方法论全在里面），且挂载校验（`standingKeyFor`）通过；预设清单是即时扫描的，无需重启即可出现。
